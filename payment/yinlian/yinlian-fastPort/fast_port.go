...
func FastPortPay(orderNo, username, ip, createTime, language string, amount, rate float64, uid int) (int, string, string) {
	if rate <= 6 && rate != 1 {
		return 0, "汇率异常", ""
	}

	p := abc.GetPaymentConfigOne("Fastport")
	...
	if sys_no == "" || secret == "" || urlValue == "" {
		telegram.SendMsg(telegram.TEXT, 0, fmt.Sprintf("支付方式%v订单%v原因%v", "Fastport", orderNo, "下单密钥参数错误"))
	...
	if res.Status != "success" {
		telegram.SendMsg(telegram.TEXT, 0, fmt.Sprintf("支付方式%v订单%v原因%v", "Fastport", orderNo, string(r)))
		return 0, golbal.Wrong[language][10052], ""
	}
